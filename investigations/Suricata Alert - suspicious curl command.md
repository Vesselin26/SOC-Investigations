...
DNS Analysis: No suspicious DNS queries were observed during the investigation period. All observed queries are towards legitimate DNS server 1.1.1.1 with no signs of malicious or newly created domains. The query behavior does not appear to be linked to any tunneling or beaconing.
 

Final assesment - No evidence of curl or scripted HTTP activity, malicious network behavior, suspicious DNS queries, file transfers. The observed activity is consistent with legitimate internal application communication (likely Splunk-related traffic). The alert is a false-positive.
