# Hello, I'm Veselin Dimitrov

Technical Account Manager with years of experience in the fintech sector, now focused on building a career as a Security Analyst with a strong interest in threat detection and incident response.

<a href="www.linkedin.com/in/veselin-dimitrov-459492179"><img src="https://img.shields.io/badge/-LinkedIn-0072b1?&style=for-the-badge&logo=linkedin&logoColor=white" /></a>

# SOC Investigations Portfolio

## Overview

This repository contains hands-on cybersecurity investigations focused on phishing analysis, SIEM detections, endpoint compromise, command-and-control activity, and incident response scenarios.

The goal of this portfolio is to demonstrate practical skills in threat detection, log analysis, IOC investigation, attack chain reconstruction, and security reporting through realistic case-based investigations.

---
## Community & Hands-On Experience

I am an active member of the **MYDFIR Forge SOC Community**, where I continuously build practical cybersecurity skills through hands-on labs and real security scenarios.

My experience includes:

- Participating in CTF challenges focused on investigation and detection  
- Solving Incident Response scenarios based on attacker activity  
- Working in a SOC simulator with realistic attacks and enterprise-style infrastructure  
- Investigating alerts using SIEM tools, endpoint telemetry, and log analysis  
- Developing skills in triage, threat detection, incident analysis, and reporting  

The investigations in this repository are based on scenarios and telemetry from this training environment.

---

## Skills

| Skill | Associated Project |
|------|--------------------|
| Security Automation & Incident Response Workflows | [AI SOC Automation Lab](projects/ai-soc-automation-lab/) |
| AI-Powered Alert Enrichment & Triage (OpenAI, Claude) | [AI SOC Automation Lab](projects/ai-soc-automation-lab/) |
| Detection Engineering, Threat Hunting & SIEM (Splunk) | [SOC Detection Lab](home-labs/SOC%20Simulation%20Lab/) |
| Network Security, Active Directory & Adversary Simulation (Zeek, Suricata, pfSense, Kali) | [SOC Detection Lab](home-labs/SOC%20Simulation%20Lab/) |
| Process Creation & Endpoint Investigation | [Suricata Alert Investigation](investigations/siem-investigations/splunk/suricata-alert/) |
| Network Traffic Analysis & C2 Detection | [Suricata Alert Investigation](investigations/siem-investigations/splunk/suricata-alert/) |
| Windows Persistence & Credential Dumping Analysis | [C2 Beaconing Investigation](investigations/siem-investigations/splunk/c2-beaconing/) |
| Incident Response & Attack Chain Reconstruction | [C2 Beaconing Investigation](investigations/siem-investigations/splunk/c2-beaconing/) |
| Static Malware Analysis (REMnux) | [Static Malware Analysis](malware-analysis/static-analysis/) |
| IOC Extraction & Malware Triage | [Static Malware Analysis](malware-analysis/static-analysis/) |
| YARA Rule Development | [YARA Rules](malware-analysis/yara-rules/) |
| Email Header Analysis | [Email Investigations](investigations/email-investigations/) |
| Phishing Detection & IOC Extraction | [Email Investigations](investigations/email-investigations/) |
| Threat Intelligence & MITRE ATT&CK Mapping | [Email Investigations](investigations/email-investigations/) |

---


## Tools & Technologies

- Network Analysis: Wireshark, Zeek, Suricata  
- SIEM: Splunk  
- Endpoint Security: Microsoft Defender  
- Malware Analysis: REMnux, FLARE VM  
- Infrastructure: Active Directory



## Certifications

<h3>📜 Certifications</h3>

<p align="left">
  <a href="https://www.credly.com/badges/73ab9d25-9b4d-433d-a7fc-38481ab57bf8/public_url">
    <img src="https://images.credly.com/size/110x110/images/80d8a06a-c384-42bf-ad36-db81bce5adce/blob" width="90"/>
  </a>

  <a href="https://www.credly.com/badges/7f30d4c0-f167-46ee-a578-c970459e9197/public_url">
    <img src="https://images.credly.com/size/110x110/images/683783d8-eaac-4c37-a14d-11bd8a36321d/ccna_600.png" width="100"/>
  </a>

  <a href="https://www.credly.com/badges/21dd1419-17e0-4205-99c8-059a29796692/public_url">
    <img src="https://images.credly.com/size/110x110/images/f6d62c5d-1e1d-4de6-92ee-8dc8c80b1c7b/blob" width="90"/>
  </a>


---

## Career Objective

Currently transitioning into cybersecurity and seeking opportunities in:

- SOC Analyst
- Security Analyst
- Incident Response
- Threat Detection
- Blue Team Operations

---

